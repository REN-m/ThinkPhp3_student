...
<form method="post" action="" name="listForm">
    ...
    <?php if(is_array($rows)): $i = 0; $__LIST__ = $rows;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr>
            ...
            <td align="center"><?php if($row['position']==1){echo '班主任'; }else{echo '任课教师'; } ?></td>
                                    <td align="center"><?php echo ($row["state"]); ?></td>
                                    <td align="center"><?php echo date('Y-m-d H:i:s',$row[create_time]);?></td>
                                    <td>
                <a href="<?php echo U('Headmaster/save',array('id'=>$row['id']));?>">编辑</a>
                |
                <a class="ajax-get" href="<?php echo U('Headmaster/del',array('id'=>$row['id']));?>">删除</a>
            </td>
