...
.explode {
  background:url(http://localhost/thinkphp3/Public/Admin/Images/menu_minus.gif) no-repeat 0px 3px;
  font-weight:bold;
}
.collapse {
  background:url(http://localhost/thinkphp3/Public/Admin/Images/menu_plus.gif) no-repeat 0px 3px;
  font-weight:bold;
}
.menu-item {
  background:url(http://localhost/thinkphp3/Public/Admin/Images/menu_arrow.gif) no-repeat 0px 3px;
  font-weight:normal;
}
#help-title {
  font-size:14px;
    ...
    <p>
        <span style="float:right; padding:3px 5px;" >
            <a href="javascript:toggleCollapse();">
                <img id="toggleImg" src="http://localhost/thinkphp3/Public/Admin/Images/menu_minus.gif" width="9" height="9" border="0" alt="闭合" />
            </a>
        </span>
...
<div id="main-div">
    <div id="menu-list">
        <ul id="menu-ul">
            <?php if(is_array($navs)): $i = 0; $__LIST__ = $navs;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$nav): $mod = ($i % 2 );++$i;?><li class="explode" key="02_cat_and_goods" name="menu">
                    <?php echo ($nav["name"]); ?>
                    <ul>
                        <?php if(is_array($menus)): $i = 0; $__LIST__ = $menus;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$menu): $mod = ($i % 2 );++$i; if($menu['nav_id'] == $nav['id']): ?><li class="menu-item"><a href="<?php echo U($menu['path']);?>" target="main-frame"><?php echo ($menu["name"]); ?></a></li><?php endif; endforeach; endif; else: echo "" ;endif; ?>
                    </ul>
...
<script type="text/javascript" src="http://localhost/thinkphp3/Public/Admin/Js/global.js"></script>
<script type="text/javascript" src="http://localhost/thinkphp3/Public/Admin/Js/utils.js"></script>
<script type="text/javascript" src="http://localhost/thinkphp3/Public/Admin/Js/transport.js"></script>
