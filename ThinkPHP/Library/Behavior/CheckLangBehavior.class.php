...
class CheckLangBehavior {
    ...
    private function checkLanguage() {
        ...
        // 读取框架语言包
        $file   =   THINK_PATH.'Lang/'.LANG_SET.'Goods.php';
        if(LANG_SET != C('DEFAULT_LANG') && is_file($file))
            L(include $file);

        // 读取应用公共语言包
        $file   =  LANG_PATH.LANG_SET.'Goods.php';
        if(is_file($file))
            L(include $file);
        
        // 读取模块语言包
        $file   =   MODULE_PATH.'Lang/'.LANG_SET.'Goods.php';
        if(is_file($file))
            L(include $file);

        // 读取当前控制器语言包
        $file   =   MODULE_PATH.'Lang/'.LANG_SET.'/'.strtolower(CONTROLLER_NAME).'Goods.php';
        if (is_file($file))
            L(include $file);
    }
}
