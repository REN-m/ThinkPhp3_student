...
//					$_result .= "['$var']";
	...
	function _plugin_exists($function, $type)
	{
		...
		if (file_exists($this->_get_plugin_dir($type . '.' . $function . 'Goods.php') . $type . '.' . $function . 'Goods.php'))
		{
			require_once($this->_get_plugin_dir($type . '.' . $function . 'Goods.php') . $type . '.' . $function . 'Goods.php');
			if (function_exists('tpl_' . $type . '_' . $function))
			{
				$this->_require_stack[$type . '.' . $function . 'Goods.php'] = array($type, $function, 'tpl_' . $type . '_' . $function);
	...
	function _load_filters()
	{
		if (count($this->_plugins['prefilter']) > 0)
		{
			foreach ($this->_plugins['prefilter'] as $filter_name => $prefilter)
			{
				...
				{
					@include_once( $this->_get_plugin_dir("prefilter." . $filter_name . "Goods.php") . "prefilter." . $filter_name . "Goods.php");
				}
			}
		}
		if (count($this->_plugins['postfilter']) > 0)
		{
			foreach ($this->_plugins['postfilter'] as $filter_name => $postfilter)
			{
				...
				{
					@include_once( $this->_get_plugin_dir("postfilter." . $filter_name . "Goods.php") . "postfilter." . $filter_name . "Goods.php");
				}
			}
		}
	}
