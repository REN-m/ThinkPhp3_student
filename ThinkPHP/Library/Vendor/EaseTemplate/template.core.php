	...
	*/
	function ETCoreStart(
		...
		if(is_dir($this->LangDir)){
			...
			if(@is_file($this->LangDir.$this->Language.'Goods.php')){
				$lang = array();
				@include_once $this->LangDir.$this->Language.'Goods.php';
				$this->LangData = $lang;
			}
		}else{
	...
	function lang(
		...
		if (is_dir($this->LangDir)){
			...
			if($this->LangData[$id]=='' && $this->Language=='default'){
				...
				if (@is_file($this->LangDir.$this->Language.'Goods.php')){
					unset($lang);
					@include($this->LangDir.$this->Language.'Goods.php');
				}
				...
				//语言文件过大时采取建立新文件
				if(strlen($docs)>400){
					$this->writer($this->LangDir.$this->Language.'.'.$id.'Goods.php','<? $etl = "'.$str.'";?>');
				...
				if($this->LangData[$id]=='o(O_O)o.ET Lang.o(*_*)o'){
					unset($etl);
					include($this->LangDir.$this->Language.".".$id."Goods.php");
	...
	function inc(
		...
		if($Files){
			...
			if($this->RunType=='Cache'){
				//引用模板
				$this->IncList[] = $Files;
				$cache_file = $this->CacheDir.$this->TplID.$Files.".".$this->Language."Goods.php";
...
<!-- END -->";
	...
	function CompilePHP(
		...
		if ($content){
			...
			if($this->RunType=='Cache' && !is_file($this->CacheDir.'Index.htm')){
				...
				$this->writer($this->CacheDir.'idea.html',$Ease_base);
							...
							if($EaseTemplate3_Cache!=''){
								...
								if (is_array($IncFile[1])){
									foreach ($IncFile[1] AS $ks=>$vs) {
										$this->IncList[] = $vs;
										//更新inc缓存
										if (@filemtime($this->TemplateDir.$vs)>@filemtime($this->CacheDir.$this->TplID.$vs.'.'.$this->Language.'Goods.php')){
											$this->inc($vs);
										}
									}
								}
	...
	*/
	function HtmCheck(
		...
		$this->HtmID = md5(trim($Name)? trim($Name).'Goods.php' : $this->Server['REQUEST_URI'].'Goods.php' );
		//检测时间
	...
	*/
	function FileName(
		...
		return $this->CacheDir.implode("_",$extdir).".".$this->Language.'Goods.php';
	}
